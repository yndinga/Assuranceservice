// <auto-generated />
using System;
using AssuranceService.Infrastructure.Data;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Metadata;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;

#nullable disable

namespace AssuranceService.Infrastructure.Migrations
{
    [DbContext(typeof(AssuranceDbContext))]
    [Migration("20260811073541_AlignAssuranceFunctionalModel")]
    partial class AlignAssuranceFunctionalModel
    {
        /// <inheritdoc />
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("ProductVersion", "8.0.10")
                .HasAnnotation("Relational:MaxIdentifierLength", 128);

            SqlServerModelBuilderExtensions.UseIdentityColumns(modelBuilder);

            modelBuilder.Entity("AssuranceService.Application.Sagas.AssuranceProcessState", b =>
                {
                    b.Property<Guid>("CorrelationId")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<bool>("AssuranceCreated")
                        .HasColumnType("bit");

                    b.Property<Guid>("AssuranceId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime?>("CompletedAt")
                        .HasColumnType("datetime2");

                    b.Property<string>("CurrentState")
                        .IsRequired()
                        .HasMaxLength(64)
                        .HasColumnType("nvarchar(64)");

                    b.Property<string>("ErrorCode")
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("ErrorMessage")
                        .HasMaxLength(1000)
                        .HasColumnType("nvarchar(1000)");

                    b.Property<bool>("GarantiesAssigned")
                        .HasColumnType("bit");

                    b.Property<string>("Importateur")
                        .IsRequired()
                        .HasMaxLength(250)
                        .HasColumnType("nvarchar(250)");

                    b.Property<DateTime?>("LastRetryAt")
                        .HasColumnType("datetime2");

                    b.Property<bool>("MarchandisesAdded")
                        .HasColumnType("bit");

                    b.Property<string>("NoPolice")
                        .IsRequired()
                        .HasMaxLength(255)
                        .HasColumnType("nvarchar(255)");

                    b.Property<bool>("PrimeCalculated")
                        .HasColumnType("bit");

                    b.Property<bool>("ProcessCompleted")
                        .HasColumnType("bit");

                    b.Property<int>("RetryCount")
                        .HasColumnType("int");

                    b.Property<DateTime>("StartedAt")
                        .HasColumnType("datetime2");

                    b.Property<string>("TypeContrat")
                        .IsRequired()
                        .HasMaxLength(25)
                        .HasColumnType("nvarchar(25)");

                    b.HasKey("CorrelationId");

                    b.ToTable("AssuranceProcessStates", (string)null);
                });

            modelBuilder.Entity("AssuranceService.Domain.Models.Aerien", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier")
                        .HasColumnName("Id");

                    b.Property<string>("AeroportDebarquementCode")
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("AeroportEmbarquementCode")
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<DateTime>("CreerLe")
                        .HasColumnType("datetime2");

                    b.Property<string>("CreerPar")
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime?>("ModifierLe")
                        .HasColumnType("datetime2");

                    b.Property<string>("ModifierPar")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("NumeroLTA")
                        .HasMaxLength(255)
                        .HasColumnType("nvarchar(255)");

                    b.Property<Guid>("VoyageId")
                        .HasColumnType("uniqueidentifier");

                    b.HasKey("Id");

                    b.HasIndex("VoyageId")
                        .IsUnique();

                    b.ToTable("Aeriens", (string)null);
                });

            modelBuilder.Entity("AssuranceService.Domain.Models.Assurance", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier")
                        .HasColumnName("Id");

                    b.Property<DateTime>("CreerLe")
                        .HasColumnType("datetime2");

                    b.Property<string>("CreerPar")
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime?>("DateDebut")
                        .HasColumnType("datetime2");

                    b.Property<DateTime?>("DateFin")
                        .HasColumnType("datetime2");

                    b.Property<string>("Duree")
                        .HasMaxLength(25)
                        .HasColumnType("nvarchar(250)");

                    b.Property<int?>("DureeJours")
                        .HasColumnType("int");

                    b.Property<string>("Etat")
                        .IsRequired()
                        .ValueGeneratedOnAdd()
                        .HasMaxLength(10)
                        .HasColumnType("nvarchar(10)")
                        .HasDefaultValue("42")
                        .HasColumnName("Statut");

                    b.Property<Guid?>("GarantieId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("ImportateurNIU")
                        .IsRequired()
                        .HasMaxLength(25)
                        .HasColumnType("nvarchar(250)");

                    b.Property<string>("ImportateurNom")
                        .IsRequired()
                        .HasMaxLength(250)
                        .HasColumnType("nvarchar(250)");

                    b.Property<string>("Intermediaire")
                        .HasMaxLength(250)
                        .HasColumnType("nvarchar(250)");

                    b.Property<string>("ModeDeTransport")
                        .IsRequired()
                        .HasMaxLength(10)
                        .HasColumnType("nvarchar(10)");

                    b.Property<DateTime?>("ModifierLe")
                        .HasColumnType("datetime2");

                    b.Property<string>("ModifierPar")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("NoFacture")
                        .HasMaxLength(250)
                        .HasColumnType("nvarchar(250)");

                    b.Property<string>("NoPolice")
                        .HasMaxLength(255)
                        .HasColumnType("nvarchar(255)");

                    b.Property<string>("NumeroAFI")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("NumeroCert")
                        .HasMaxLength(25)
                        .HasColumnType("nvarchar(250)");

                    b.Property<string>("OCRE")
                        .IsRequired()
                        .HasMaxLength(250)
                        .HasColumnType("nvarchar(250)");

                    b.Property<string>("PCRE")
                        .IsRequired()
                        .HasMaxLength(250)
                        .HasColumnType("nvarchar(250)");

                    b.Property<string>("Partenaire")
                        .HasMaxLength(250)
                        .HasColumnType("nvarchar(250)");

                    b.Property<string>("TypeContrat")
                        .IsRequired()
                        .HasMaxLength(25)
                        .HasColumnType("nvarchar(250)");

                    b.Property<string>("TypePartenaire")
                        .IsRequired()
                        .ValueGeneratedOnAdd()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)")
                        .HasDefaultValue("ASSUREUR");

                    b.HasKey("Id");

                    b.HasIndex("GarantieId");

                    b.HasIndex("NoPolice")
                        .IsUnique()
                        .HasFilter("[NoPolice] IS NOT NULL");

                    b.HasIndex("NumeroAFI")
                        .IsUnique();

                    b.HasIndex("NumeroCert")
                        .IsUnique()
                        .HasFilter("[NumeroCert] IS NOT NULL");

                    b.ToTable("Assurances", (string)null);
                });

            modelBuilder.Entity("AssuranceService.Domain.Models.AssuranceDeclaration", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier")
                        .HasColumnName("Id");

                    b.Property<Guid>("AssuranceId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime>("CreerLe")
                        .HasColumnType("datetime2");

                    b.Property<string>("CreerPar")
                        .HasColumnType("nvarchar(max)");

                    b.Property<Guid>("DeclarationId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime?>("ModifierLe")
                        .HasColumnType("datetime2");

                    b.Property<string>("ModifierPar")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("NumeroDI")
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<string>("PaysEmbarquementCode")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("PortEmbarquementCode")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.HasKey("Id");

                    b.HasIndex("DeclarationId");

                    b.HasIndex("AssuranceId", "DeclarationId")
                        .IsUnique();

                    b.ToTable("AssuranceDeclarations", (string)null);
                });

            modelBuilder.Entity("AssuranceService.Domain.Models.AssuranceLigne", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier")
                        .HasColumnName("Id");

                    b.Property<Guid>("AssuranceDeclarationId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid>("AssuranceId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("Colisage")
                        .IsRequired()
                        .HasMaxLength(250)
                        .HasColumnType("nvarchar(250)");

                    b.Property<DateTime>("CreerLe")
                        .HasColumnType("datetime2");

                    b.Property<string>("CreerPar")
                        .HasColumnType("nvarchar(max)");

                    b.Property<Guid>("DeclarationId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("Designation")
                        .IsRequired()
                        .HasMaxLength(250)
                        .HasColumnType("nvarchar(250)");

                    b.Property<string>("Devise")
                        .HasMaxLength(20)
                        .HasColumnType("nvarchar(20)");

                    b.Property<bool>("EstPartielle")
                        .HasColumnType("bit");

                    b.Property<string>("Marque")
                        .IsRequired()
                        .HasMaxLength(250)
                        .HasColumnType("nvarchar(250)");

                    b.Property<decimal?>("MasseBrute")
                        .HasPrecision(18, 5)
                        .HasColumnType("decimal(18,5)");

                    b.Property<decimal?>("MasseNette")
                        .HasPrecision(18, 5)
                        .HasColumnType("decimal(18,5)");

                    b.Property<DateTime?>("ModifierLe")
                        .HasColumnType("datetime2");

                    b.Property<string>("ModifierPar")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int>("NoLigne")
                        .HasColumnType("int");

                    b.Property<string>("PaysOrigine")
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("PositionTarifaire")
                        .IsRequired()
                        .HasMaxLength(11)
                        .HasColumnType("nvarchar(11)");

                    b.Property<decimal?>("PrixUnitaire")
                        .HasPrecision(18, 5)
                        .HasColumnType("decimal(18,5)");

                    b.Property<decimal?>("Quantite")
                        .HasPrecision(18, 5)
                        .HasColumnType("decimal(18,5)");

                    b.Property<Guid>("SourceCommandeId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid>("SourceLigneDIId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("UniteStatistique")
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<decimal?>("ValeurDevise")
                        .HasPrecision(18, 5)
                        .HasColumnType("decimal(18,5)");

                    b.Property<decimal?>("ValeurXAF")
                        .HasPrecision(18, 5)
                        .HasColumnType("decimal(18,5)");

                    b.Property<decimal?>("Volume")
                        .HasPrecision(18, 5)
                        .HasColumnType("decimal(18,5)");

                    b.HasKey("Id");

                    b.HasIndex("AssuranceDeclarationId");

                    b.HasIndex("SourceLigneDIId");

                    b.HasIndex("AssuranceId", "SourceLigneDIId")
                        .IsUnique();

                    b.ToTable("AssuranceLignes", (string)null);
                });

            modelBuilder.Entity("AssuranceService.Domain.Models.Avenant", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier")
                        .HasColumnName("Id");

                    b.Property<Guid>("AssuranceId")
                        .HasColumnType("uniqueidentifier")
                        .HasColumnName("AssuranceId");

                    b.Property<DateTime>("CreerLe")
                        .HasColumnType("datetime2");

                    b.Property<string>("CreerPar")
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime?>("ModifierLe")
                        .HasColumnType("datetime2");

                    b.Property<string>("ModifierPar")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Motif")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("NoAvenant")
                        .IsRequired()
                        .HasMaxLength(255)
                        .HasColumnType("nvarchar(255)");

                    b.Property<string>("NoPolice")
                        .IsRequired()
                        .HasMaxLength(255)
                        .HasColumnType("nvarchar(255)");

                    b.Property<string>("Statut")
                        .IsRequired()
                        .HasMaxLength(10)
                        .HasColumnType("nvarchar(10)");

                    b.Property<string>("Type")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)")
                        .HasColumnName("Type");

                    b.HasKey("Id");

                    b.HasIndex("AssuranceId", "NoAvenant")
                        .IsUnique();

                    b.ToTable("Avenants", (string)null);
                });

            modelBuilder.Entity("AssuranceService.Domain.Models.Commentaire", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier")
                        .HasColumnName("Id");

                    b.Property<DateTime>("CreerLe")
                        .HasColumnType("datetime2");

                    b.Property<string>("CreerPar")
                        .HasColumnType("nvarchar(max)");

                    b.Property<Guid>("DocumentId")
                        .HasColumnType("uniqueidentifier")
                        .HasColumnName("DocumentId");

                    b.Property<DateTime?>("ModifierLe")
                        .HasColumnType("datetime2");

                    b.Property<string>("ModifierPar")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Motif")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("motif");

                    b.HasKey("Id");

                    b.ToTable("Commentaires", (string)null);
                });

            modelBuilder.Entity("AssuranceService.Domain.Models.Cotation", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier")
                        .HasColumnName("Id");

                    b.Property<decimal?>("Accessoires")
                        .HasColumnType("decimal(18,2)");

                    b.Property<decimal>("ContratId")
                        .HasColumnType("decimal(20,0)");

                    b.Property<DateTime>("CreerLe")
                        .HasColumnType("datetime2");

                    b.Property<string>("CreerPar")
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime?>("ModifierLe")
                        .HasColumnType("datetime2");

                    b.Property<string>("ModifierPar")
                        .HasColumnType("nvarchar(max)");

                    b.Property<decimal>("Montant")
                        .HasColumnType("decimal(18,2)");

                    b.Property<decimal>("PartenaireId")
                        .HasColumnType("decimal(20,0)");

                    b.Property<decimal?>("PrimeNette")
                        .HasColumnType("decimal(18,2)");

                    b.Property<decimal?>("PrimeTotale")
                        .HasColumnType("decimal(18,2)");

                    b.Property<decimal>("StatutId")
                        .HasColumnType("decimal(20,0)");

                    b.Property<string>("Taux")
                        .HasMaxLength(255)
                        .HasColumnType("nvarchar(255)");

                    b.Property<decimal?>("Taxe")
                        .HasColumnType("decimal(18,2)");

                    b.Property<decimal>("UserId")
                        .HasColumnType("decimal(20,0)");

                    b.HasKey("Id");

                    b.ToTable("Cotations", (string)null);
                });

            modelBuilder.Entity("AssuranceService.Domain.Models.Document", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier")
                        .HasColumnName("Id");

                    b.Property<Guid>("AssuranceId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("ContentType")
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<DateTime>("CreerLe")
                        .HasColumnType("datetime2");

                    b.Property<string>("CreerPar")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Description")
                        .IsRequired()
                        .HasMaxLength(255)
                        .HasColumnType("nvarchar(255)");

                    b.Property<string>("DocumentUrl")
                        .IsRequired()
                        .HasMaxLength(500)
                        .HasColumnType("nvarchar(500)")
                        .HasColumnName("ObjectKey");

                    b.Property<DateTime?>("ModifierLe")
                        .HasColumnType("datetime2");

                    b.Property<string>("ModifierPar")
                        .HasColumnType("nvarchar(max)");

                    b.Property<long?>("Taille")
                        .HasColumnType("bigint")
                        .HasColumnName("FileSize");

                    b.Property<string>("TypeDocument")
                        .IsRequired()
                        .ValueGeneratedOnAdd()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)")
                        .HasDefaultValue("PIECE_ASSURANCE");

                    b.HasKey("Id");

                    b.HasIndex("AssuranceId");

                    b.ToTable("Documents", (string)null);
                });

            modelBuilder.Entity("AssuranceService.Domain.Models.Fluvial", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier")
                        .HasColumnName("Id");

                    b.Property<DateTime>("CreerLe")
                        .HasColumnType("datetime2");

                    b.Property<string>("CreerPar")
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime?>("ModifierLe")
                        .HasColumnType("datetime2");

                    b.Property<string>("ModifierPar")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("NomNavire")
                        .HasMaxLength(255)
                        .HasColumnType("nvarchar(255)");

                    b.Property<string>("PortDebarquementCode")
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("PortEmbarquementCode")
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("TypeNavire")
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<Guid>("VoyageId")
                        .HasColumnType("uniqueidentifier");

                    b.HasKey("Id");

                    b.HasIndex("VoyageId")
                        .IsUnique();

                    b.ToTable("Fluviaux", (string)null);
                });

            modelBuilder.Entity("AssuranceService.Domain.Models.Garantie", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier")
                        .HasColumnName("Id");

                    b.Property<decimal>("Accessoires")
                        .HasColumnType("decimal(18,2)");

                    b.Property<bool>("Actif")
                        .HasColumnType("bit");

                    b.Property<DateTime>("CreerLe")
                        .HasColumnType("datetime2");

                    b.Property<string>("CreerPar")
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime?>("ModifierLe")
                        .HasColumnType("datetime2");

                    b.Property<string>("ModifierPar")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Nom")
                        .IsRequired()
                        .HasMaxLength(255)
                        .HasColumnType("nvarchar(255)")
                        .HasColumnName("NomGarantie");

                    b.Property<decimal?>("Taux")
                        .HasPrecision(18, 4)
                        .HasColumnType("decimal(18,4)");

                    b.HasKey("Id");

                    b.ToTable("Garanties", (string)null);
                });

            modelBuilder.Entity("AssuranceService.Domain.Models.Historique", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier")
                        .HasColumnName("Id");

                    b.Property<Guid>("AssuranceId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid>("AvenantId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("CibleEntite")
                        .IsRequired()
                        .HasMaxLength(30)
                        .HasColumnType("nvarchar(30)");

                    b.Property<string>("Commentaire")
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime>("CreerLe")
                        .HasColumnType("datetime2");

                    b.Property<string>("CreerPar")
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime?>("ModifierLe")
                        .HasColumnType("datetime2");

                    b.Property<string>("ModifierPar")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("NomChamp")
                        .IsRequired()
                        .HasMaxLength(200)
                        .HasColumnType("nvarchar(200)");

                    b.Property<Guid?>("ReferenceId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("ValeurApres")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("ValeurAvant")
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("Id");

                    b.HasIndex("AssuranceId");

                    b.HasIndex("AvenantId");

                    b.ToTable("Historiques", (string)null);
                });

            modelBuilder.Entity("AssuranceService.Domain.Models.Maritime", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier")
                        .HasColumnName("Id");

                    b.Property<DateTime>("CreerLe")
                        .HasColumnType("datetime2");

                    b.Property<string>("CreerPar")
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime?>("ModifierLe")
                        .HasColumnType("datetime2");

                    b.Property<string>("ModifierPar")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("NomNavire")
                        .HasMaxLength(255)
                        .HasColumnType("nvarchar(255)");

                    b.Property<string>("NumeroBL")
                        .HasMaxLength(255)
                        .HasColumnType("nvarchar(255)");

                    b.Property<string>("PortDebarquementCode")
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("PortEmbarquementCode")
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("TypeNavire")
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<Guid>("VoyageId")
                        .HasColumnType("uniqueidentifier");

                    b.HasKey("Id");

                    b.HasIndex("VoyageId")
                        .IsUnique();

                    b.ToTable("Maritimes", (string)null);
                });

            modelBuilder.Entity("AssuranceService.Domain.Models.Prime", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier")
                        .HasColumnName("Id");

                    b.Property<decimal?>("Accessoires")
                        .HasColumnType("decimal(18,2)");

                    b.Property<Guid>("AssuranceId")
                        .HasColumnType("uniqueidentifier")
                        .HasColumnName("AssuranceId");

                    b.Property<DateTime>("CreerLe")
                        .HasColumnType("datetime2");

                    b.Property<string>("CreerPar")
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime?>("ModifierLe")
                        .HasColumnType("datetime2");

                    b.Property<string>("ModifierPar")
                        .HasColumnType("nvarchar(max)");

                    b.Property<decimal?>("PrimeNette")
                        .HasColumnType("decimal(18,2)");

                    b.Property<decimal?>("PrimeTotale")
                        .HasColumnType("decimal(18,2)");

                    b.Property<string>("Statut")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("Statut");

                    b.Property<decimal?>("Taux")
                        .HasPrecision(18, 4)
                        .HasColumnType("decimal(18,4)");

                    b.Property<decimal?>("Taxe")
                        .HasColumnType("decimal(18,2)");

                    b.Property<decimal>("ValeurDevise")
                        .HasColumnType("decimal(18,2)");

                    b.Property<decimal>("ValeurFCFA")
                        .HasColumnType("decimal(18,2)");

                    b.HasKey("Id");

                    b.HasIndex("AssuranceId");

                    b.ToTable("Primes", (string)null);
                });

            modelBuilder.Entity("AssuranceService.Domain.Models.Referentiel.Aeroport", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier")
                        .HasColumnName("Id");

                    b.Property<bool>("Actif")
                        .HasColumnType("bit");

                    b.Property<string>("Code")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime>("CreerLe")
                        .HasColumnType("datetime2");

                    b.Property<string>("CreerPar")
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime?>("ModifierLe")
                        .HasColumnType("datetime2");

                    b.Property<string>("ModifierPar")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Nom")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<Guid?>("PaysId")
                        .HasColumnType("uniqueidentifier");

                    b.HasKey("Id");

                    b.ToTable("Aeroports");
                });

            modelBuilder.Entity("AssuranceService.Domain.Models.Referentiel.Corridor", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier")
                        .HasColumnName("Id");

                    b.Property<bool>("Actif")
                        .HasColumnType("bit");

                    b.Property<string>("Code")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime>("CreerLe")
                        .HasColumnType("datetime2");

                    b.Property<string>("CreerPar")
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime?>("ModifierLe")
                        .HasColumnType("datetime2");

                    b.Property<string>("ModifierPar")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Nom")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<Guid?>("PaysId")
                        .HasColumnType("uniqueidentifier");

                    b.HasKey("Id");

                    b.ToTable("Corridors");
                });

            modelBuilder.Entity("AssuranceService.Domain.Models.Referentiel.Departement", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier")
                        .HasColumnName("Id");

                    b.Property<bool>("Actif")
                        .HasColumnType("bit");

                    b.Property<string>("Code")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime>("CreerLe")
                        .HasColumnType("datetime2");

                    b.Property<string>("CreerPar")
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime?>("ModifierLe")
                        .HasColumnType("datetime2");

                    b.Property<string>("ModifierPar")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Nom")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<int>("Ordre")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.ToTable("Departements");
                });

            modelBuilder.Entity("AssuranceService.Domain.Models.Referentiel.Devise", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier")
                        .HasColumnName("Id");

                    b.Property<bool>("Actif")
                        .HasColumnType("bit");

                    b.Property<string>("Code")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime>("CreerLe")
                        .HasColumnType("datetime2");

                    b.Property<string>("CreerPar")
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime?>("ModifierLe")
                        .HasColumnType("datetime2");

                    b.Property<string>("ModifierPar")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Nom")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.HasKey("Id");

                    b.ToTable("Devises");
                });

            modelBuilder.Entity("AssuranceService.Domain.Models.Referentiel.Etat", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier")
                        .HasColumnName("Id");

                    b.Property<bool>("Actif")
                        .HasColumnType("bit");

                    b.Property<int>("Code")
                        .HasColumnType("int");

                    b.Property<string>("CodeEcran")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("nvarchar(20)");

                    b.Property<DateTime>("CreerLe")
                        .HasColumnType("datetime2");

                    b.Property<string>("CreerPar")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Description")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Libelle")
                        .IsRequired()
                        .HasMaxLength(150)
                        .HasColumnType("nvarchar(150)");

                    b.Property<DateTime?>("ModifierLe")
                        .HasColumnType("datetime2");

                    b.Property<string>("ModifierPar")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("UsageUI")
                        .IsRequired()
                        .HasMaxLength(250)
                        .HasColumnType("nvarchar(250)");

                    b.HasKey("Id");

                    b.HasIndex("Code")
                        .IsUnique();

                    b.HasIndex("CodeEcran")
                        .IsUnique();

                    b.ToTable("Etats");
                });

            modelBuilder.Entity("AssuranceService.Domain.Models.Referentiel.Pays", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier")
                        .HasColumnName("Id");

                    b.Property<bool>("Actif")
                        .HasColumnType("bit");

                    b.Property<string>("Code")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime>("CreerLe")
                        .HasColumnType("datetime2");

                    b.Property<string>("CreerPar")
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime?>("ModifierLe")
                        .HasColumnType("datetime2");

                    b.Property<string>("ModifierPar")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Nom")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.HasKey("Id");

                    b.ToTable("Pays");
                });

            modelBuilder.Entity("AssuranceService.Domain.Models.Referentiel.Port", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier")
                        .HasColumnName("Id");

                    b.Property<bool>("Actif")
                        .HasColumnType("bit");

                    b.Property<string>("Code")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime>("CreerLe")
                        .HasColumnType("datetime2");

                    b.Property<string>("CreerPar")
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime?>("ModifierLe")
                        .HasColumnType("datetime2");

                    b.Property<string>("ModifierPar")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Module")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("Nom")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<Guid?>("PaysId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("Type")
                        .HasMaxLength(2)
                        .HasColumnType("nvarchar(2)");

                    b.HasKey("Id");

                    b.ToTable("Ports", (string)null);
                });

            modelBuilder.Entity("AssuranceService.Domain.Models.Referentiel.Route", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier")
                        .HasColumnName("Id");

                    b.Property<bool>("Actif")
                        .HasColumnType("bit");

                    b.Property<string>("Code")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime>("CreerLe")
                        .HasColumnType("datetime2");

                    b.Property<string>("CreerPar")
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime?>("ModifierLe")
                        .HasColumnType("datetime2");

                    b.Property<string>("ModifierPar")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Nom")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.HasKey("Id");

                    b.ToTable("Routes");
                });

            modelBuilder.Entity("AssuranceService.Domain.Models.Referentiel.RouteNationale", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier")
                        .HasColumnName("Id");

                    b.Property<bool>("Actif")
                        .HasColumnType("bit");

                    b.Property<string>("Code")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("nvarchar(20)");

                    b.Property<DateTime>("CreerLe")
                        .HasColumnType("datetime2");

                    b.Property<string>("CreerPar")
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime?>("ModifierLe")
                        .HasColumnType("datetime2");

                    b.Property<string>("ModifierPar")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Nom")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.HasKey("Id");

                    b.ToTable("RoutesNationales");
                });

            modelBuilder.Entity("AssuranceService.Domain.Models.Referentiel.Statut", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier")
                        .HasColumnName("Id");

                    b.Property<string>("Code")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<DateTime>("CreerLe")
                        .HasColumnType("datetime2");

                    b.Property<string>("CreerPar")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Libelle")
                        .IsRequired()
                        .HasMaxLength(250)
                        .HasColumnType("nvarchar(250)");

                    b.Property<DateTime?>("ModifierLe")
                        .HasColumnType("datetime2");

                    b.Property<string>("ModifierPar")
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("Id");

                    b.ToTable("Statuts");
                });

            modelBuilder.Entity("AssuranceService.Domain.Models.Referentiel.TauxDeChange", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier")
                        .HasColumnName("Id");

                    b.Property<bool>("Actif")
                        .HasColumnType("bit");

                    b.Property<DateTime>("CreerLe")
                        .HasColumnType("datetime2");

                    b.Property<string>("CreerPar")
                        .HasColumnType("nvarchar(max)");

                    b.Property<Guid>("DeviseId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime?>("ModifierLe")
                        .HasColumnType("datetime2");

                    b.Property<string>("ModifierPar")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Source")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<decimal>("Taux")
                        .HasPrecision(20, 5)
                        .HasColumnType("decimal(20, 5)");

                    b.Property<string>("ValideDe")
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.HasKey("Id");

                    b.HasIndex("DeviseId");

                    b.ToTable("TauxDeChanges", (string)null);
                });

            modelBuilder.Entity("AssuranceService.Domain.Models.Referentiel.Troncon", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier")
                        .HasColumnName("Id");

                    b.Property<bool>("Actif")
                        .HasColumnType("bit");

                    b.Property<string>("Code")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime>("CreerLe")
                        .HasColumnType("datetime2");

                    b.Property<string>("CreerPar")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Description")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<DateTime?>("ModifierLe")
                        .HasColumnType("datetime2");

                    b.Property<string>("ModifierPar")
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("Id");

                    b.ToTable("Troncons");
                });

            modelBuilder.Entity("AssuranceService.Domain.Models.Referentiel.TypePartenaire", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier")
                        .HasColumnName("Id");

                    b.Property<bool>("Actif")
                        .HasColumnType("bit");

                    b.Property<string>("Code")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<DateTime>("CreerLe")
                        .HasColumnType("datetime2");

                    b.Property<string>("CreerPar")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Description")
                        .HasMaxLength(500)
                        .HasColumnType("nvarchar(500)");

                    b.Property<string>("Libelle")
                        .IsRequired()
                        .HasMaxLength(150)
                        .HasColumnType("nvarchar(150)");

                    b.Property<DateTime?>("ModifierLe")
                        .HasColumnType("datetime2");

                    b.Property<string>("ModifierPar")
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("Id");

                    b.HasIndex("Code")
                        .IsUnique();

                    b.ToTable("TypePartenaires", (string)null);
                });

            modelBuilder.Entity("AssuranceService.Domain.Models.Referentiel.TypeTransport", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier")
                        .HasColumnName("Id");

                    b.Property<bool>("Actif")
                        .HasColumnType("bit");

                    b.Property<string>("Code")
                        .IsRequired()
                        .HasMaxLength(3)
                        .HasColumnType("nvarchar(3)");

                    b.Property<DateTime>("CreerLe")
                        .HasColumnType("datetime2");

                    b.Property<string>("CreerPar")
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime?>("ModifierLe")
                        .HasColumnType("datetime2");

                    b.Property<string>("ModifierPar")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Nom")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.HasKey("Id");

                    b.ToTable("TypeTransports");
                });

            modelBuilder.Entity("AssuranceService.Domain.Models.Referentiel.UniteStatistique", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier")
                        .HasColumnName("Id");

                    b.Property<bool>("Actif")
                        .HasColumnType("bit");

                    b.Property<string>("Code")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("nvarchar(20)");

                    b.Property<DateTime>("CreerLe")
                        .HasColumnType("datetime2");

                    b.Property<string>("CreerPar")
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime?>("ModifierLe")
                        .HasColumnType("datetime2");

                    b.Property<string>("ModifierPar")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Nom")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.HasKey("Id");

                    b.ToTable("UniteStatistiques");
                });

            modelBuilder.Entity("AssuranceService.Domain.Models.Routier", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier")
                        .HasColumnName("Id");

                    b.Property<DateTime>("CreerLe")
                        .HasColumnType("datetime2");

                    b.Property<string>("CreerPar")
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime?>("ModifierLe")
                        .HasColumnType("datetime2");

                    b.Property<string>("ModifierPar")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("NumeroLV")
                        .HasMaxLength(255)
                        .HasColumnType("nvarchar(255)");

                    b.Property<string>("RouteNationaleCode")
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<Guid>("VoyageId")
                        .HasColumnType("uniqueidentifier");

                    b.HasKey("Id");

                    b.HasIndex("VoyageId")
                        .IsUnique();

                    b.ToTable("Routiers", (string)null);
                });

            modelBuilder.Entity("AssuranceService.Domain.Models.Specificite", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier")
                        .HasColumnName("Id");

                    b.Property<DateTime>("CreerLe")
                        .HasColumnType("datetime2");

                    b.Property<string>("CreerPar")
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime?>("ModifierLe")
                        .HasColumnType("datetime2");

                    b.Property<string>("ModifierPar")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Nom")
                        .IsRequired()
                        .HasMaxLength(255)
                        .HasColumnType("nvarchar(255)");

                    b.HasKey("Id");

                    b.ToTable("Specificites", (string)null);
                });

            modelBuilder.Entity("AssuranceService.Domain.Models.VisaAssurance", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier")
                        .HasColumnName("Id");

                    b.Property<Guid>("AssuranceId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime>("CreerLe")
                        .HasColumnType("datetime2");

                    b.Property<string>("CreerPar")
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime?>("DateVisa")
                        .HasColumnType("datetime2(0)");

                    b.Property<string>("Licence")
                        .HasMaxLength(250)
                        .HasColumnType("nvarchar(250)");

                    b.Property<string>("Message")
                        .HasMaxLength(1000)
                        .HasColumnType("nvarchar(1000)");

                    b.Property<DateTime?>("ModifierLe")
                        .HasColumnType("datetime2");

                    b.Property<string>("ModifierPar")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Organisation")
                        .IsRequired()
                        .HasMaxLength(250)
                        .HasColumnType("nvarchar(250)");

                    b.Property<string>("Statut")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("TypePartenaire")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("VisaContent")
                        .HasColumnType("nvarchar(max)");

                    b.Property<bool>("VisaOK")
                        .HasColumnType("bit");

                    b.HasKey("Id");

                    b.HasIndex("AssuranceId", "TypePartenaire", "Organisation")
                        .IsUnique();

                    b.ToTable("VisaAssurances", (string)null);
                });

            modelBuilder.Entity("AssuranceService.Domain.Models.Voyage", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier")
                        .HasColumnName("Id");

                    b.Property<Guid>("AssuranceId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("Conditionnement")
                        .HasMaxLength(500)
                        .HasColumnType("nvarchar(500)");

                    b.Property<DateTime>("CreerLe")
                        .HasColumnType("datetime2");

                    b.Property<string>("CreerPar")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("DescriptionConditionnement")
                        .HasMaxLength(500)
                        .HasColumnType("nvarchar(500)");

                    b.Property<string>("Designation")
                        .IsRequired()
                        .HasMaxLength(255)
                        .HasColumnType("nvarchar(255)");

                    b.Property<string>("Devise")
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("DureeSejour")
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("LieuSejour")
                        .HasMaxLength(255)
                        .HasColumnType("nvarchar(255)");

                    b.Property<string>("Marque")
                        .HasMaxLength(255)
                        .HasColumnType("nvarchar(255)");

                    b.Property<string>("MasseBrute")
                        .HasMaxLength(255)
                        .HasColumnType("nvarchar(255)");

                    b.Property<DateTime?>("ModifierLe")
                        .HasColumnType("datetime2");

                    b.Property<string>("ModifierPar")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Nature")
                        .HasMaxLength(500)
                        .HasColumnType("nvarchar(500)");

                    b.Property<string>("NomTransporteur")
                        .IsRequired()
                        .HasMaxLength(255)
                        .HasColumnType("nvarchar(255)");

                    b.Property<string>("PaysDestination")
                        .IsRequired()
                        .HasMaxLength(255)
                        .HasColumnType("nvarchar(255)");

                    b.Property<string>("PaysProvenance")
                        .IsRequired()
                        .HasMaxLength(255)
                        .HasColumnType("nvarchar(255)");

                    b.Property<string>("Specificites")
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<string>("UniteStatistique")
                        .HasMaxLength(255)
                        .HasColumnType("nvarchar(255)");

                    b.HasKey("Id");

                    b.HasIndex("AssuranceId")
                        .IsUnique();

                    b.ToTable("Voyages", (string)null);
                });

            modelBuilder.Entity("AssuranceService.Domain.Models.Aerien", b =>
                {
                    b.HasOne("AssuranceService.Domain.Models.Voyage", "Voyage")
                        .WithOne("Aerien")
                        .HasForeignKey("AssuranceService.Domain.Models.Aerien", "VoyageId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Voyage");
                });

            modelBuilder.Entity("AssuranceService.Domain.Models.Assurance", b =>
                {
                    b.HasOne("AssuranceService.Domain.Models.Garantie", "Garantie")
                        .WithMany()
                        .HasForeignKey("GarantieId")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.Navigation("Garantie");
                });

            modelBuilder.Entity("AssuranceService.Domain.Models.AssuranceDeclaration", b =>
                {
                    b.HasOne("AssuranceService.Domain.Models.Assurance", "Assurance")
                        .WithMany("Declarations")
                        .HasForeignKey("AssuranceId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Assurance");
                });

            modelBuilder.Entity("AssuranceService.Domain.Models.AssuranceLigne", b =>
                {
                    b.HasOne("AssuranceService.Domain.Models.AssuranceDeclaration", "AssuranceDeclaration")
                        .WithMany("Lignes")
                        .HasForeignKey("AssuranceDeclarationId")
                        .OnDelete(DeleteBehavior.NoAction)
                        .IsRequired();

                    b.HasOne("AssuranceService.Domain.Models.Assurance", "Assurance")
                        .WithMany("Lignes")
                        .HasForeignKey("AssuranceId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Assurance");

                    b.Navigation("AssuranceDeclaration");
                });

            modelBuilder.Entity("AssuranceService.Domain.Models.Avenant", b =>
                {
                    b.HasOne("AssuranceService.Domain.Models.Assurance", "Assurance")
                        .WithMany()
                        .HasForeignKey("AssuranceId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Assurance");
                });

            modelBuilder.Entity("AssuranceService.Domain.Models.Document", b =>
                {
                    b.HasOne("AssuranceService.Domain.Models.Assurance", "Assurance")
                        .WithMany("Documents")
                        .HasForeignKey("AssuranceId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Assurance");
                });

            modelBuilder.Entity("AssuranceService.Domain.Models.Fluvial", b =>
                {
                    b.HasOne("AssuranceService.Domain.Models.Voyage", "Voyage")
                        .WithOne("Fluvial")
                        .HasForeignKey("AssuranceService.Domain.Models.Fluvial", "VoyageId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Voyage");
                });

            modelBuilder.Entity("AssuranceService.Domain.Models.Historique", b =>
                {
                    b.HasOne("AssuranceService.Domain.Models.Avenant", "Avenant")
                        .WithMany("Historiques")
                        .HasForeignKey("AvenantId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Avenant");
                });

            modelBuilder.Entity("AssuranceService.Domain.Models.Maritime", b =>
                {
                    b.HasOne("AssuranceService.Domain.Models.Voyage", "Voyage")
                        .WithOne("Maritime")
                        .HasForeignKey("AssuranceService.Domain.Models.Maritime", "VoyageId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Voyage");
                });

            modelBuilder.Entity("AssuranceService.Domain.Models.Prime", b =>
                {
                    b.HasOne("AssuranceService.Domain.Models.Assurance", null)
                        .WithMany("Primes")
                        .HasForeignKey("AssuranceId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("AssuranceService.Domain.Models.Referentiel.TauxDeChange", b =>
                {
                    b.HasOne("AssuranceService.Domain.Models.Referentiel.Devise", "Devise")
                        .WithMany("TauxDeChanges")
                        .HasForeignKey("DeviseId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("Devise");
                });

            modelBuilder.Entity("AssuranceService.Domain.Models.Routier", b =>
                {
                    b.HasOne("AssuranceService.Domain.Models.Voyage", "Voyage")
                        .WithOne("Routier")
                        .HasForeignKey("AssuranceService.Domain.Models.Routier", "VoyageId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Voyage");
                });

            modelBuilder.Entity("AssuranceService.Domain.Models.VisaAssurance", b =>
                {
                    b.HasOne("AssuranceService.Domain.Models.Assurance", "Assurance")
                        .WithMany("Visas")
                        .HasForeignKey("AssuranceId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Assurance");
                });

            modelBuilder.Entity("AssuranceService.Domain.Models.Voyage", b =>
                {
                    b.HasOne("AssuranceService.Domain.Models.Assurance", "Assurance")
                        .WithOne("Voyage")
                        .HasForeignKey("AssuranceService.Domain.Models.Voyage", "AssuranceId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Assurance");
                });

            modelBuilder.Entity("AssuranceService.Domain.Models.Assurance", b =>
                {
                    b.Navigation("Declarations");

                    b.Navigation("Documents");

                    b.Navigation("Lignes");

                    b.Navigation("Primes");

                    b.Navigation("Visas");

                    b.Navigation("Voyage");
                });

            modelBuilder.Entity("AssuranceService.Domain.Models.AssuranceDeclaration", b =>
                {
                    b.Navigation("Lignes");
                });

            modelBuilder.Entity("AssuranceService.Domain.Models.Avenant", b =>
                {
                    b.Navigation("Historiques");
                });

            modelBuilder.Entity("AssuranceService.Domain.Models.Referentiel.Devise", b =>
                {
                    b.Navigation("TauxDeChanges");
                });

            modelBuilder.Entity("AssuranceService.Domain.Models.Voyage", b =>
                {
                    b.Navigation("Aerien");

                    b.Navigation("Fluvial");

                    b.Navigation("Maritime");

                    b.Navigation("Routier");
                });
#pragma warning restore 612, 618
        }
    }
}
