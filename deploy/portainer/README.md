...
**Tag figé (recommandé prod)** : le YAML utilise par défaut **`ghcr.io/yndinga/assuranceservice:v1.1.0`**.

1. Crée et pousse le tag Git (déclenche la CI et publie l’image sur GHCR) :
...
2. Pour la release suivante : `v1.1.1`, etc., puis dans Portainer mets **`ASSURANCE_IMAGE=ghcr.io/yndinga/assuranceservice:v1.1.1`**.

*(Option « toujours la dernière branche » : `ASSURANCE_IMAGE=ghcr.io/yndinga/assuranceservice:master` ou `:latest` — ce n’est plus un tag figé.)*
...
- **Environment variables** : définir au minimum :
  - `ASSURANCE_IMAGE` (optionnel : défaut **`ghcr.io/yndinga/assuranceservice:v1.1.0`**)
...
2. **Tag d’image figé** : normal qu’il ne change pas après un push sur `master` seul. Nouvelle version : tag Git `v1.1.1` + mets à jour **`ASSURANCE_IMAGE`** (ou **`IMAGE_TAG`** dans `portainer-stack.yml`).
...
4. **Mode Swarm** : `pull_image` / politique de pull peut être différente ; en dernier recours sur le nœud :  
   `docker service update --force --image ghcr.io/yndinga/assuranceservice:v1.1.0 NOM_DU_SERVICE`
5. **En SSH sur l’hôte Docker** (diagnostic) :

```bash
docker pull ghcr.io/yndinga/assuranceservice:v1.1.0
